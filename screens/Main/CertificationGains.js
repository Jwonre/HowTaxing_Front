...
const CertificationGains = props => {
        ...
        if (response.data.data && response.data.data.length > 0) {
          ...
          if (list.some(item => item.complete === false)) {
            setTimeout(async () => {
              ...
              navigation.push('FixedHouseList', { isGainsTax: true, chatListindex: props?.route?.params?.index });
            }, 300);
            return false;
          } else {
...
export default CertificationGains;
